...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  if(!(ETH_WritePHYRegister(PHYAddress, PHY_BCR, PHY_Reset)))
  {
    ...
    printf(" <<<<<<< ETH config [Put the PHY in reset mode] ERROR <<<<<<<\r\n");
    return ETH_ERROR;
  }
  ...
  {  
    ...
    {
      printf(" <<<<<<< ETH config [wait for linked satus] ERROR <<<<<<<\r\n");
      return ETH_ERROR;
    }
    ...
    {
      /* Return ERROR in case of write timeout */
      printf(" <<<<<<< ETH config [Enable Auto-Negotiation] ERROR <<<<<<<\r\n");
      return ETH_ERROR;
    }
    ...
    if(timeout == PHY_READ_TO)
    {
      printf(" <<<<<<< ETH config [Wait autonegotiation completed] ERROR <<<<<<<\r\n");      
      return ETH_ERROR;
    }
    /* Reset Timeout counter */
    timeout = 0;
