...
uint32_t CPAL_I2C_Init(CPAL_InitTypeDef* pDevInitStruct) 
{
    ...
    /* Disable I2Cx Device */
    __CPAL_I2C_HAL_DISABLE_DEV(pDevInitStruct->CPAL_Dev); //i2c_1
...
{   
  ...
  else if ((pDevInitStruct->CPAL_State == CPAL_STATE_BUSY_RX) && (pDevInitStruct->pCPAL_TransferRx->wNumData == 2))
  {
    /* Disable Acknowledge */
    __CPAL_I2C_HAL_DISABLE_ACK(pDevInitStruct->CPAL_Dev);
