...
#define ETH_LINK_TIME 2 //以分钟为单位
#define ETH_RECV_TIME 2

#define LSE_READ_ERROR    ((uint32_t)0x1FFFF)  //TYH: RTC等待'LSE'超时 
...
uint8_t RTC_Init(uint8_t clk_source)
{
  ...
  {
    timeout++;
  } while ( (RCC_GetFlagStatus(RCC_FLAG_LSERDY) == RESET) && (timeout < LSE_READ_TO) );
